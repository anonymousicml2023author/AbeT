...
def save_log(prefix, output_dir, date_str, rank=0):
    ...
    logging.basicConfig(
        level=logging.INFO, format=fmt, datefmt=date_fmt, filename=filename, filemode="w"
    )
...
def evaluate_eval(
    ...
    print(f"~~~\nsaving last snapshot to {last_snapshot}\n~~~")
    ...
    if mean_iu > args.best_record["mean_iu"]:
        ...
        print(f"~~~\nupdating best snapshot to {best_snapshot}\n~~~")
        ...
        for bs_idx, bs_data in enumerate(dump_images):
            for local_idx, data in enumerate(zip(bs_data[0], bs_data[1], bs_data[2])):
                ...
                val_visual.extend(
                    [visualize(gt_pil.convert("RGB")), visualize(predictions_pil.convert("RGB"))]
                )
                if local_idx >= 9:
                    break
    ...
    # logging.info('-' * 107)
    print("=" * 100)
    print(f" ~~~ \nbest record: {args.best_record}\n ~~~")
    print("=" * 100)
    fmt_str = (
        "[epoch %d], [val loss %.5f], [acc %.5f], [acc_cls %.5f], "
        + "[mean_iu %.5f], [fwavacc %.5f]"
    )
    logging.info(fmt_str % (epoch, val_loss.avg, acc, acc_cls, mean_iu, fwavacc))
    fmt_str = (
        "best record: [val loss %.5f], [acc %.5f], [acc_cls %.5f], "
