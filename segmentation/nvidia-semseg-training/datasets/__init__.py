...
from datasets import camvid, cityscapes, kitti, mapillary
...
def setup_loaders(args):
    ...
    if args.jointwtborder:
        target_train_transform = extended_transforms.RelaxedBoundaryLossToTensor(
            args.dataset_cls.ignore_label, args.dataset_cls.num_classes
        )
    else:
    ...
    if args.dataset == "cityscapes":
        ...
        if args.class_uniform_pct:
            if args.coarse_boost_classes:
                coarse_boost_classes = [int(c) for c in args.coarse_boost_classes.split(",")]
            else:
