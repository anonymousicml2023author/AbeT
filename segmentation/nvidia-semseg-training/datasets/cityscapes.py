...
def add_items(items, aug_items, cities, img_path, mask_path, mask_postfix, mode, maxSkip):
    ...
    for c in cities:
        c_items = [
            name.split("_leftImg8bit.png")[0] for name in os.listdir(os.path.join(img_path, c))
        ]
        for it in c_items:
            item = (
            ...
            if mode == "train" and maxSkip > 0:
                ...
                prev_item = (
                    os.path.join(new_img_path, c, prev_it + "_leftImg8bit.png"),
                ...
                next_item = (
                    os.path.join(new_img_path, c, next_it + "_leftImg8bit.png"),
...
def make_dataset(quality, mode, maxSkip=0, fine_coarse_mult=6, cv_split=0):
    ...
    if quality == "coarse":
        ...
        add_items(
            items, aug_items, coarse_dirs[mode], img_path, mask_path, mask_postfix, mode, maxSkip
        )
    elif quality == "fine":
        ...
        for mode in modes:
            if mode == "test":
                ...
                add_items(
                    items, aug_items, cv_splits, img_path, mask_path, mask_postfix, mode, maxSkip
                )
            else:
...
class CityScapes(data.Dataset):
    def __init__(
        ...
        if cv_split:
            ...
            ), "expected cv_split {} to be < CV_SPLITS {}".format(cv_split, cfg.DATASET.CV_SPLITS)
        else:
            self.cv_split = 0
        ...
        if self.class_uniform_pct > 0:
            if cut:
                ...
                self.imgs_uniform = uniform.build_epoch(
                    self.imgs, self.fine_centroids, num_classes, cfg.CLASS_UNIFORM_PCT
                )
            else:
                self.imgs_uniform = uniform.build_epoch(
                    self.imgs + self.aug_imgs, self.centroids, num_classes, cfg.CLASS_UNIFORM_PCT
                )
        else:
