...
from utils.misc import AverageMeter, evaluate_eval, fast_hist, prep_experiment
...
parser.add_argument(
    "--dataset", type=str, default="cityscapes", help="cityscapes, mapillary, camvid, kitti"
)
...
parser.add_argument(
    "--class_uniform_tile", type=int, default=1024, help="tile size for class uniform sampling"
)
parser.add_argument(
...
parser.add_argument(
    "--img_wt_loss", action="store_true", default=False, help="per-image class-weighted loss"
)
parser.add_argument(
    "--batch_weighting",
    action="store_true",
...
parser.add_argument(
    "--jointwtborder", action="store_true", default=False, help="Enable boundary label relaxation"
)
parser.add_argument(
    "--strict_bdr_cls",
    type=str,
...
parser.add_argument(
    "--apex", action="store_true", default=False, help="Use Nvidia Apex Distributed Data Parallel"
)
...
parser.add_argument(
    "--gblur", action="store_true", default=True, help="Use Guassian Blur Augmentation"
)
parser.add_argument(
    "--bblur", action="store_true", default=False, help="Use Bilateral Blur Augmentation"
)
...
parser.add_argument(
    "--scale_max", type=float, default=2.0, help="dynamically scale training images up to this size"
)
...
parser.add_argument(
    "--maxSkip", type=int, default=0, help="Skip x number of  frames of video augmented dataset"
)
...
def main():
    ...
    args.temperature_model = args.temperature_model.lower()
    assert args.temperature_model in ["none", "baseline", "learned"]
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        # scheduler.step()
        train(train_loader, net, optim, criterion, epoch, writer, scaler=scaler)
        scheduler.step()
...
def train(train_loader, net, optim, criterion, curr_epoch, writer, scaler=None):
    """
    ...
    if args.local_rank == 0:
        gen = tqdm(enumerate(train_loader), total=len(train_loader))
    else:
    ...
    for i, data in gen:
        ...
        if args.fp16:
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                ...
                if isinstance(outputs, tuple):
                    outputs = outputs[0]  # adding to handle temperature outputs
        ...
        if not torch.isnan(log_main_loss):
            train_main_loss.update(log_main_loss.item(), batch_pixel_size)
        else:
            print(f"\n ~~~ {args.local_rank} NAN LOSS ~~~")

        if args.fp16:
            # with amp.scale_loss(main_loss, optim) as scaled_loss:
            #     scaled_loss.backward()
            # grad scaler handles not stepping on inf/nan grads
...
def validate(val_loader, net, criterion, optim, curr_epoch, writer):
    ...
    if args.local_rank == 0:
        gen = tqdm(enumerate(val_loader), total=len(val_loader))
    else:
        gen = enumerate(val_loader)

    # for val_idx, data in enumerate(val_loader):
    for val_idx, data in gen:
            ...
            if args.fp16:
                with torch.autocast(device_type="cuda", dtype=torch.float16):
                    ...
                    if isinstance(output, tuple):
                        output = output[0]  # adding to handle temperature output
                    loss = criterion(output, gt_cuda).item()
            else:
                output = net(inputs)
                if isinstance(output, tuple):
                    output = output[0]  # adding to handle temperature output
        ...
        if not np.isnan(loss):
            ...
            print(f" ~~~ {args.local_rank} NAN LOSS ~~~")
        predictions = output.data.max(1)[1].cpu()

        # Logging
        if args.local_rank == 0:
            msg = "[epoch {}], [iter {} / {}], [val loss {:0.6f}]".format(
                curr_epoch,
                val_idx + 1,
                len(val_loader),
                val_loss.avg,
        ...
        iou_acc += fast_hist(
            predictions.numpy().flatten(), gt_image.numpy().flatten(), args.dataset_cls.num_classes
        )
    ...
    if args.local_rank == 0:
        evaluate_eval(
            args, net, optim, val_loss, iou_acc, dump_images, writer, curr_epoch, args.dataset_cls
        )
