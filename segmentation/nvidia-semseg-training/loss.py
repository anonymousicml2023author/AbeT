...
def get_loss(args):
    ...
    criterion_val = CrossEntropyLoss2d(
        size_average=True, weight=None, ignore_index=args.dataset_cls.ignore_label
    ).cuda()
    return criterion, criterion_val
...
class ImageBasedCrossEntropyLoss2d(nn.Module):
    ...
    def __init__(
        self, classes, weight=None, size_average=True, ignore_index=255, norm=False, upper_bound=1.0
    ):
        ...
        self.nll_loss = nn.NLLLoss(
            weight, reduction="mean" if size_average else "sum", ignore_index=ignore_index
        )
        self.norm = norm
        ...
        if self.fp16:
            loss_matrix = (
                ...
                * (
                    ...
                    * customsoftmax(inputs, target[:, :-1, :, :].half())
                ).sum(1)
            ) * (1.0 - mask.half())
        else:
            loss_matrix = (
                ...
                * (
                    target[:, :-1, :, :].float()
                    * class_weights.unsqueeze(0).unsqueeze(2).unsqueeze(3)
                    * customsoftmax(inputs, target[:, :-1, :, :].float())
                ).sum(1)
            ) * (1.0 - mask.float())
