...
parser.add_argument(
    ...
    default="SDCNet2D",
    choices=model_names,
    help="model architecture: " + " | ".join(model_names) + " (default: SDCNet2D)",
)
parser.add_argument(
    "-s",
    "--save",
...
# Resources
parser.add_argument(
    "-j", "--workers", default=4, type=int, help="number of data loader workers (default: 10)"
)
...
parser.add_argument(
    ...
    help="Scheduler for learning" + " rate (only ExponentialLR, MultiStepLR, PolyLR supported.",
)
parser.add_argument(
    "--lr_gamma", default=0.1, type=float, help="learning rate will be multipled by this gamma"
)
parser.add_argument(
    "--lr_step", default=200, type=int, help="stepsize of changing the learning rate"
)
parser.add_argument(
    "--lr_milestones",
    type=int,
...
parser.add_argument(
    "--seed", default=1234, type=int, metavar="SEED", help="seed for initializing training. "
)
parser.add_argument(
    "--optimizer",
    default="Adam",
...
parser.add_argument(
    "--val_file", metavar="VALIDATION_FILE", help="validation file (default : None)"
)
parser.add_argument(
    "--val_batch_size", type=int, default=1, help="Batch size to use for validation."
)
parser.add_argument(
    "--val_n_batches",
    default=-1,
...
parser.add_argument(
    "--video_fps", type=int, default=30, help="Render predicted video with a specified frame rate"
)
parser.add_argument("--val_freq", default=500000, type=int, help="frequency of running validation")
parser.add_argument(
    "--stride",
    default=64,
    type=int,
...
parser.add_argument(
    "--sample_rate", type=int, default=1, help="step size in looping through datasets"
)
parser.add_argument("--start_epoch", type=int, default=-1, help="Set epoch number during resuming")
parser.add_argument(
    "--skip_aug",
    action="store_true",
    help="Skips expensive geometric or photometric augmentations.",
)

parser.add_argument(
    "--rgb_max", type=float, default=255, help="maximum expected value of rgb colors"
)
...
parser.add_argument(
    "--write_video", action="store_true", help="save video to 'args.save/args.name.mp4'."
)

parser.add_argument(
    "--eval", action="store_true", help="Run model in inference or evaluation mode."
)
...
def get_train_and_valid_data_loaders(block, args):
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, sampler=train_sampler, shuffle=(train_sampler is None), **tkwargs
    )

    block.log(
        "Number of Training Images: {}:{}".format(len(train_loader.dataset), len(train_loader))
    )
...
def get_learning_rate_scheduler(optimizer, block, args):
    if args.lr_scheduler == "ExponentialLR":
        block.log(
            "using exponential decay learning rate with " + "{} decay rate".format(args.lr_gamma)
        )
        lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, args.lr_gamma)
    elif args.lr_scheduler == "MultiStepLR":
        block.log(
            "using multi-step learning rate with {} gamma"
...
def evaluate(model, val_loader, block, args, epoch):
    ...
    if args.rank == 0 and args.write_video:
        _pipe = tools.create_pipe(
            os.path.join(args.save_root, "__epoch_%03d.mp4" % (epoch)),
            gWidth,
            gHeight,
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(tqdm(val_loader, total=num_batches)):
            ...
            for b in range(args.val_batch_size):
                ...
                pred_image = (output_images[b].data.cpu().numpy().transpose(1, 2, 0)).astype(
                    np.uint8
                )
    ...
    if args.rank == 0 and args.write_video and args.eval:
        shutil.move(
            os.path.join(args.save_root, "__epoch_%03d.mp4" % (epoch)),
            os.path.join(
                args.save_root, "_%s_%03d_psnr_%1.2f.mp4" % (args.name, epoch, avg_metrics[0])
            ),
        )
    ...
    block.log(
        "max memory allocated (GB): {:.3f}: ".format(
...
def train_epoch(
    ...
    num_batches = len(train_loader) if args.train_n_batches < 0 else args.train_n_batches
    ...
    for i, batch in enumerate(train_loader):
        ...
        loss, outputs, _ = train_step(
            batch, model, optimizer, block, args, ((global_index + 1) % args.print_freq == 0)
        )
...
def save_model(model, optimizer, epoch, global_index, max_psnr, block, args):
    ...
    if args.rank == 0:
        ...
        for k in state_dict:
            [
                tmp_keys.pop(k) if (k in tmp_keys and ikey in k) else None
                for ikey in model_.ignore_keys
            ]
        ...
        model_name = os.path.join(
            args.save_root,
            "_ckpt_epoch_%03d_iter_%07d_psnr_%1.2f.pt.tar" % (epoch, global_index, max_psnr),
        )
...
def train(model, optimizer, lr_scheduler, train_loader, train_sampler, val_loader, block, args):
    ...
    if args.eval:
        ...
        v_psnr, v_ssim, v_ie, v_loss = evaluate(
            model, val_loader, block, args, args.start_epoch + 1
        )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.save_freq == 0:
            ...
            if v_psnr > max_psnr:
                max_psnr = v_psnr
                save_model(model, optimizer, epoch + 1, global_index, max_psnr, block, args)
...
def main():
    ...
    with tools.TimerBlock(
        "Building {} Model and {} Optimizer".format(args.model, args.optimizer_class.__name__)
    ) as block:
    ...
    with tools.TimerBlock("Training Model") as block:
        train(model, optimizer, lr_scheduler, train_loader, train_sampler, val_loader, block, args)
